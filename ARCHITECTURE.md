...
- **Self-metrics** (via the OTel-Go SDK, all `genai_otel_bridge_`-prefixed — the authoritative
  ...
  `samples_capped_total`, `new_label_values_total`, `source_capability_total` (closed
  `{loop,graph,state}`), `source_data_incomplete_total` (closed `{loop,reason}`),
...
| 17 | **AWS ECS deployment target via DynamoDB-for-both** (alongside K8s — amends #6/#7's "external lock/store, addable later"). One DynamoDB table backs BOTH leader election and the checkpoint, behind the FROZEN `coordinate.Coordinator` + `checkpoint.Checkpointer` interfaces — one binary, one image, selected by `ha.coordinator=dynamodb`/`ha.checkpoint=dynamodb` (+ `ha.dynamodb.*`, `helm:"omit"` so it stays out of the K8s chart). The lock is a conditional-write CAS `UpdateItem`; a monotonic `fence` Number attribute is the leader epoch (the `Lease.LeaseTransitions` analogue, bumped once per acquire of an empty/expired lock, read once at election via `coordinate.WithEpoch`); renew is `holder=:me AND fence=:myFence` and never bumps `fence`. The checkpoint mirrors the ConfigMap RMW (GetItem → `CheckMonotonic` → conditional PutItem on a `version` attr); **time is stored as an RFC3339Nano string** (NOT UnixNano — a zero `time.Time` must round-trip to `IsZero()` for the logs-export cursor-only watermark). **Safety (at-most-one-advancer) is clock-independent** — it rests on the CAS + the epoch fence, neither of which reads a clock; only failover *timing* (`expiresAtMs < now`) is clock-sensitive, and here it is **more** clock-sensitive than K8s, not identical: `acquire` compares the CANDIDATE's wall clock against an `expiresAtMs` written from the LEADER's wall clock, so failover timing depends on **absolute inter-node clock skew** between the two, bounded roughly by `lease_duration − renew_deadline` (~5s with the 15s/10s defaults) — a candidate whose clock runs that far ahead of the leader's can steal an actively-renewed lock. client-go's K8s follower instead compares its own locally-observed last-change time against its own clock, so absolute skew cancels out and only clock-**rate** drift matters. This backend therefore depends on NTP-synced hosts in a way the K8s Lease path does not. The lock item is **never auto-expired** (no DynamoDB TTL — parity with the K8s Lease, which is never auto-deleted) so a long full outage can't reset `fence` below a surviving checkpoint's epoch and permanently fence writes; expiry is the `expiresAtMs` comparison only. ECS specifics: distroless `-healthcheck` binary mode for the container health check; Task-ARN identity from the task-metadata endpoint; Fargate `stopTimeout` max 120s (vs K8s 300s) is safe because the lock needs no release (it expires; the fence prevents double-emit). Delivered on a feature branch + PR (OSS practice). | `cirello.io/dynamolock` (Apache-2.0, license-OK, but exposes only an opaque GUID RVN — **no monotonic fence token**, the exact contract needed; would require layering our own counter anyway → rolled our own ~150 LOC); single-task `desired_count=1` no-coordination (drops active/standby — relies on ECS reschedule, minutes not seconds); EFS-mounted file checkpoint (needs fsync + cross-writer CAS hardening the file backend lacks); UnixNano time encoding (corrupts the zero-time watermark) |
| 18 | **Split source capability from data incompleteness at the injected `source.Deps` seam.** | `OnCapability(loop,graph,CapabilityState)` feeds `source_capability_total{loop,graph,state}`; `OnDataIncomplete(loop,IncompleteReason)` feeds `source_data_incomplete_total{loop,reason}`. Both label vocabularies are closed Go types, while the schedule seam deliberately stays string-only and the composition root adapts. A complete breaking migration retired the old mixed counter because nine of its fourteen producers were not 404/capability events. `endpoint-absent` and `plan-unsupported` remain enumerated but producerless: one 404 cannot distinguish them from a transient route failure, so permanent versus flapping stays a query-time property. | Dual emit or deprecation window (unnecessary migration complexity); one renamed mixed metric (preserves the false semantics); deriving permanent absence or plan gates from a single 404 (unsupported inference) |
