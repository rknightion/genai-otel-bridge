...
`templates/_helpers.tpl` defines `decant.labels` / `decant.selectorLabels` — the common label block is
included everywhere (DRY). **`selectorLabels` is the immutable Deployment/PDB selector — kept minimal
(`app: decant`); never fold version/instance into it or `helm upgrade` breaks.**
...
- `templates/deployment.yaml` — non-root (UID 65532), seccomp RuntimeDefault, pod topology spread by
  ...
  **`terminationGracePeriod 300s`** to cover the emit-retry budget. `resources` is a standard
  ...
  rolls the pods — it hashes ONLY the `decant-config` CM, NOT the app-created `decant-checkpoints` state
...
- `templates/rbac.yaml` — **Role, not ClusterRole**, namespace-scoped AND **resource-name-scoped**:
  `leases` get/update only on `decant-leader`, `configmaps` get/update only on `decant-checkpoints`
  (so the pod can't touch its own `decant-config`). `create` stays unscoped — RBAC `resourceNames`
...
- `templates/cleanup-hook.yaml` — **`post-delete` uninstall cleanup.** The binary creates the
  `decant-leader` Lease and `decant-checkpoints` ConfigMap at runtime, so `helm uninstall` doesn't
  track them and would orphan them. This hook (gated by `cleanup.enabled`, default true) runs a Job
  `decant -cleanup` to delete them. **post-delete, not pre-delete** — pre-delete runs while the
...
- `grafana/self-obs/` (the integrator's own o11y, `decant_*`) — **deploy-by-default, push the whole dir:**
  - **Dashboard** `decant-self-obs`: v2 `TabsLayout` + responsive `AutoGridLayout`, 7 tabs (Overview/SLO,
  ...
  - **recording rules**: `decant:last_success_age:seconds`, `:baseline6h`, `decant:freshness_ratio`,
    `decant:upstream_error_ratio:5m` (the dynamic-threshold seam the dashboard colours on) + the originals
    `decant:window_truncated:rate5m`, `decant:scrape_healthy`, `decant:scrape_present`.
...
- **Per-bucket gauge semantics (non-obvious):** `portkey_api_*` are gauges — aggregate with `sum_over_time`,
  **never `rate()`/`increase()`** (those are for counters; `decant_source_graph_unavailable_total` and the
  `decant_upstream_request_duration_seconds` histogram ARE counters, so `rate()`/`increase()` are correct there).
  Note the queue metric is `decant_queue_depth_ratio` (the OTLP unit-1 `_ratio` suffix), not `_depth`. Full
...
- **GS3** — exempt the `decant_*` namespace from Adaptive Metrics aggregation (else self-metrics get
  aggregated away).
