# Design spec — `aip-oi` v1 (tracked design-of-record)
...
- **Buckets are eventually-consistent**: a recent bucket's value can change after first observation
  ...
  `aip_oi_bucket_revised_after_settle_total` (followup.md §8).
...
- **Distinct resource identity (H4):** self-telemetry uses its own `service.namespace`
  (e.g. `aip-oi-meta`), never the product identity — otherwise the gateway synthesises the same `job`
...
- **OP5d — `bucket_settle` measured on a live workspace (2026-06-19).** Max post-close late-arrival
  ...
  detected**: `aip_oi_bucket_revised_after_settle_total{loop}` fires when a settled bucket changes —
