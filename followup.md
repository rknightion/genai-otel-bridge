...
- **Multi-workspace / multi-project targeting + auto-discovery (Portkey, and parity for LangSmith).**
  ...
  **✅ SAFETY GUARDRAIL BUILT 2026-06-21 (`scope.go`, `settings.expected_workspace`):** since per-request
  ...
  `OnCapability(loop,"workspace_scope",CapabilityPermissionDenied)`, recovering without restart (resilient posture, a design decision). `logs_export` is unaffected — it already passes `workspace_id`, which Portkey DOES
...
| **Capability and incomplete-data attribution** | ✅ RESOLVED. The complete migration uses typed `OnCapability` and `OnDataIncomplete` hooks, with `genai_otel_bridge_source_capability_total{loop,graph,state}` and `genai_otel_bridge_source_data_incomplete_total{loop,reason}`. All fourteen former mixed-event producers have discriminating tests. 404s use `transient-404`; permanent absence versus flapping remains a query-time distinction. | round3-#4; gob-0006 |
...
| **Auth-failure observability** (`auth_errors_total`) | Poller self-telemetry distinguishing API failure types | ✅ **BUILT 2026-06-21.** `genai_otel_bridge_auth_errors_total{loop,source}` (selfobs counter) fires via `Deps.OnAuthError(loop,source)` (same injected-hook pattern as `OnCapability`/`OnDataIncomplete`/`OnBucketRevised`; wired in `app.go`) when an upstream responds 401/403 — a credential failure is now its OWN alertable signal (`alert on rate(...) > 0`), separable from the generic 4xx/timeout the `upstream_request_duration_seconds{status_class}` histogram lumps together. `source.IsAuthStatus(code)` is the shared 401/403 predicate. Instrumented at the four GET-based loop status-check sites that hold the raw code: portkey `analytics`+`groups`, langsmith `sessions`+`runs`. Tests: one hook-fires test per loop + a selfobs counter/attribute test. **Deliberate coverage gap (documented):** the portkey `logs_export` job lifecycle (via `logs_client`, which returns a string error not a code) is NOT yet instrumented — a lifecycle 401 still surfaces loudly via `window_lag` + `source_data_incomplete_total{reason="export_failed"}`. Thread the code through `logs_client`/`runs_client` if logs-loop auth needs its own counter. | Deliberate coverage gap; follow up if logs-loop auth needs its own counter. | design analysis |
| **Full `source_capability{state}` metric** | Distinguish endpoint-absent / plan-unsupported / permission-denied / no-data / transient-404 / schema-changed | ✅ **BUILT in gob-0006.** `genai_otel_bridge_source_capability_total{loop,graph,state}` has a closed typed state set. `endpoint-absent` and `plan-unsupported` are enumerated but producerless because one 404 cannot support that inference; current 404s use `transient-404`. Non-capability loss and quality events use the separate closed `source_data_incomplete_total{loop,reason}` counter. | Complete breaking migration; no dual emit. | design analysis; gob-0006 |
...
|---|---|---|
| **Emit pipeline (`loop.emit`) — cross-queue propagation** | ✅ BUILT (2026-09-22). Each metrics and logs OTLP POST attempt records `genai_otel_bridge_emit_request_duration_seconds{plane,status_class}` through the existing observer, including retries. The worker starts `loop.emit` as a child of the enqueueing `loop.tick` around batch processing. | A private schedule-local queue envelope carries the unchanged `model.Batch` plus a copied `trace.SpanContext`; cancellation and lease epoch come from the live leader context, never a retained tick context. Response-body reads and retry backoff are outside the per-attempt histogram. |
...
and can we recover it? Investigated with live probes + a code read; the durable conclusions and
reviewed disposition are recorded below. Probe scripts are not shipped with the repository.
