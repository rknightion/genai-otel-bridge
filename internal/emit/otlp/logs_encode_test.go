...
func TestEncodeLogsDeterministic(t *testing.T) {
	...
	id1 := map[string]string{"service.namespace": "genai-otel-bridge-meta", "deployment.environment.name": "dev"}
	id2 := map[string]string{"deployment.environment.name": "dev", "service.namespace": "genai-otel-bridge-meta"}
	l1 := []model.LogRecord{
...
func TestEncodeLogsGroupsByIndexedAttrs(t *testing.T) {
	ts := time.Unix(1_700_000_000, 0).UTC()
	id := map[string]string{"service.namespace": "genai-otel-bridge-meta"}
	...
	for _, rl := range ld.ResourceLogs {
		...
		if attrs["service.namespace"] != "genai-otel-bridge-meta" {
...
func TestEncodeLogsAttributeMapping(t *testing.T) {
	...
	if res["ai_model"] != "gpt-5" || res["service.namespace"] != "genai-otel-bridge-meta" {
