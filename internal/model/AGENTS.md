...
- `LogRecord.TraceID` carries a source-provided correlation id (a Portkey metadata `correlation_id`,
...
- `Fingerprint` is order-insensitive (it sorts internally). Adding or removing a series changes the
  fingerprint, and since the fingerprint is part of `CheckpointKey` the new series set bootstraps its
