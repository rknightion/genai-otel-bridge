...
func Signals() []signal.Signal {
	return []signal.Signal{
		...
		selfMetric("source_capability_total", "counter", "1", "source capability or permission condition by graph and closed state: endpoint-absent, plan-unsupported, permission-denied, no-data, transient-404, or schema-changed. endpoint-absent and plan-unsupported are enumerated but currently producerless because one 404 cannot distinguish them from a transient failure; steady versus intermittent increments remain a query-time judgement", "loop", "graph", "state"),
		selfMetric("source_data_incomplete_total", "counter", "1", "source data was incomplete for a closed reason: window_truncated, sessions_truncated, backfill_skipped, window_oversize, span_stats_unavailable, duplicate_dimension, trace_id_unparsed, line_oversize, line_unparseable, export_stuck, or export_failed", "loop", "reason"),
		...
		selfMetric("upstream_request_duration_seconds", "histogram", "s", "base2 exponential histogram: outbound request latency to upstream source APIs (time to response headers)", "target", "method", "status_class"),
		selfMetric("emit_request_duration_seconds", "histogram", "s", "base2 exponential histogram: outbound OTLP emit request latency (per POST attempt to /v1/metrics or /v1/logs)", "plane", "status_class"),
		selfMetric("bucket_revised_after_settle_age_seconds", "histogram", "s", "base2 exponential histogram: age (now − bucketEnd) of a settled bucket observed to change after bucket_settle", "loop"),
		{
