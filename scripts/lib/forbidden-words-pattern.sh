#!/usr/bin/env bash
# Shared forbidden-words pattern builder — sourced by scripts/forbidden-words.sh (full-tree scan,
# pre-commit + CI on the repo's own trusted checkout). Kept as its own lib so the pattern lives in one
# place.
#
# Sets $PATTERN from:
...
#      gitignored scripts/forbidden-words.local. Absent (no secret, fresh clone) → credential shapes only.
...
# shellcheck disable=SC2034  # consumed by the sourcing script (forbidden-words.sh)
