...
#
# Default target is the built binary (bin/genai-otel-bridge), which embeds the Go module
# build info — i.e. exactly the modules linked into the release. Set SBOM_TARGET to scan
# something else (e.g. an image ref `ghcr.io/owner/genai-otel-bridge:vX.Y.Z` or `dir:.`).
...
#   SBOM_TARGET   what syft scans (default: bin/genai-otel-bridge)
#   OUT_DIR       output directory (default: dist/sbom)
#   SBOM_NAME     output basename (default: genai-otel-bridge)
set -euo pipefail

SYFT="${SYFT:-syft}"
SBOM_TARGET="${SBOM_TARGET:-bin/genai-otel-bridge}"
OUT_DIR="${OUT_DIR:-dist/sbom}"
SBOM_NAME="${SBOM_NAME:-genai-otel-bridge}"
